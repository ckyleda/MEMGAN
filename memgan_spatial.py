...
from gen_fake_vms import gen_images
import progressbar
...
def extract_mem_layer(x):
    # Shape is [batch, h, w, c]
    shape = K.int_shape(x)
    modify = tf.image.resize_images(x, (10, 10))
    modify = modify[:, :, :, 1]
    #modify = K.reshape(modify, (-1, shape[1]*shape[2]))
    #modify = K.mean(modify, axis=-1)
    #modify = K.expand_dims(modify)
...
def memorability_aux(res):
    ...
    model = Lambda(extract_mem_layer)(model)
    model = Flatten()(model)
...
def construct_models(lod, a, G_old=None, D_old=None, load=False, epoch=0):
    ...
    GAN = multi_gpu_model(GAN, gpus=4)
...
grow_checkpoints = [10, 30, 50, 70, 90, 180, 1000]
batch_sizes = [256, 128, 128, 8, 4, 4, 4]
...
for epoch in range(*EPOCHS):
    ...
    for index in progressbar.progressbar(range(int(batches_per_epoch))):
        #print("Starting discriminator training loop")
        mem_noise = gen_images(BATCH_SIZE)
        ...
        mem_noise = gen_images(BATCH_SIZE)
        g_loss = GAN.train_on_batch([noise, mem_noise], [np.ones((BATCH_SIZE, 1)), mem_noise])
